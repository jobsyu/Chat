...
@interface VCardViewController()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate,EditVCardViewControllerDelegate>
{
    // 文本标签的标题数组
...
-(void)viewDidLoad
{
    ...
    _titleLablesArray= @[@[_headImageView,_nickNameText,_jidText],@[_orgNameText,_orgUnitText,_titleText,_photoNumberText,_emailText]];
...
{
    // 1.获取当前账号的电子名片
    XMPPvCardTemp *myCard = [[[self appDelegate] xmppvCardTempModule] myvCardTemp];
    ...
    //更新或保存电子名片
    [[[self appDelegate] xmppvCardTempModule] updateMyvCardTemp:myCard];
...
#pragma mark EditVCardViewControllerDelegate 代理方法
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(NSIndexPath *)indexPath
{
    ...
    //传递内容标签
    controller.contentLabel = _titleLablesArray[indexPath.section][indexPath.row];
    //3.设置代理
...
-(void)savevCard
{
    ...
    myCard.orgUnits =@[_orgUnitText.text];
